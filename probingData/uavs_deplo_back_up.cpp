...
typedef struct sln{
	...
	vector<vector<double>> dists;// squared matrix of distances between uavs. Type igraph to save space
...
printf("uav %d,", i);
			do
			{
				...
				int randindex=1+rand()%net->gcovs[i].size();
				int induav=net->gcovs[i][randindex];
				double *buff=new double[dim];
				for(j=0;j<dim;j++)
					buff[dim]=net->uavs[induav][j];
				net->uavs.push_back(buff);
				net->uavcovs[induav].push_back(i);// new uav covers ground node i
				net->gcovs[i].push_back(net->uavs.size()-1);// Include new uav as covering ground g

			/* Keep duplicating until constraint on lb is satisfied */
			}while(net->gcovs[i].size() < lb);
		}
	}
printf("\n");
};
...
double* solve_linear_model(sln* net, double range, double lb)
{
	...
	glp_prob *lp;
	int ia[nbr_grnds*net->uavs.size()],ja[nbr_grnds*net->uavs.size()];
	double ar[nbr_grnds*net->uavs.size()], z=0.;
	...
	glp_add_cols(lp, net->uavs.size()-1);
	for(j=1;j<net->uavs.size();j++)
	{
		sprintf(buff,"x%d",j);
...
printf("RANGE %f, lb %f\n",range,lb);
	...
	glp_load_matrix(lp, nbr_grnds*(net->uavs.size()-1), ia, ja, ar);
	

	int ind[net->uavs.size()];
	double val[net->uavs.size()];
for(j=1;j<net->uavs.size();j++)
{
	ind[j]=0;
	val[j]=0;
...
//	z = glp_get_obj_val(lp);
	...
	double sumrows=0, maxrow=0, minrow=net->n_uavs-1, tmp=0, lessThanAvrg=0, moreThanvrg=0;// Needed for stats on covers (constraints)
...
*/
	...
	for(i=1;i<=nbr_grnds;i++)
	{
		...
		{
			indice=net->gcovs[i][j];// find index of uav
...
sln* method1ePasse(double** input_data, int nbr_grnds, double threshold)
{
	...
	sln* res=new sln;
	res->gcovs=new vector<int>[nbr_grnds+1];
	double *buffdouble=new double[dim];
	res->uavs.push_back(buffdouble);// indices from 1 to n, so this to initialise a dummy value
	res->uavcovs.push_back(vector<int>(1,-1));// same as previous, dummy value
	res->dists.push_back(vector<double>(1,-1.));/* same again, but remember it's a square matrix so for every uav added, will
												always have to add a new vector of size the vector 'uavs' */
	...
	res->gcovs[1][0]=1;// label of first ground node : first cluster
	...
	for(k=2; k<=nbr_grnds; k++)
	{
		...
		{// closest centroid is within admissible range => add element to cluster
			...
			for(j=0;j<dim;j++)
			{
				...
				res->uavs[res->gcovs[k][0]][j]=cl[res->gcovs[k][0]][j]/res->uavcovs.size();
			}
		}
		else
		{// closest centroid is not within admissible range => Create own new cluster
			res->gcovs[k][0]=res->uavcovs.size();
			res->uavcovs.push_back(vector<int>(1,k));
			buffdouble=new double[dim];
			for(j=0;j<dim;j++)
	...
	for(k=1;k<res->uavs.size();k++)
		res->dists.push_back(vector<double>(res->uavs.size(),-1.));
